...
const ImportProjectModal: React.FC<ImportProjectModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal-950/80 px-4 backdrop-blur-sm">
      <div className="w-full max-w-lg border border-white/[0.08] bg-charcoal-900 p-6 shadow-2xl shadow-black/40">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-text-primary">{panelTitle}</h2>
          <button type="button" className="btn-ghost h-8 px-3" onClick={onClose}>
            Close
          </button>
        </div>
        ...
        {!mode && (
          <div className="space-y-2">
            <button type="button" className="btn-secondary w-full justify-start px-4" onClick={() => setMode('zip')}>
              Import .zip
            </button>
            <button type="button" className="btn-secondary w-full justify-start px-4" onClick={() => setMode('github')}>
              Import from GitHub
            </button>
            <button type="button" className="btn-secondary w-full justify-start px-4 opacity-60" disabled>
        ...
        <div className="mt-5 border-t border-white/[0.08] pt-4">
          <button type="button" className="btn-ghost h-8 px-3" onClick={() => setShowLogs((open) => !open)}>
            {showLogs ? 'Hide logs' : 'Show logs'}
          </button>
          {showLogs && (
            <pre className="mt-2 max-h-40 overflow-y-auto border border-white/[0.08] bg-charcoal-850 p-3 font-mono text-xs text-text-secondary">
